# Advisor de clustering no BigQuery

> GCP 009 · intermediário · desempenho e custos

Ordena até quatro clustering columns pela frequência dos filtros e cardinalidade.
...
| Item | Função |
|---|---|
| Python/JSON | Análise local do workload |
| BigQuery | Warehouse futuro |
| CLUSTER BY | Organizar blocos relacionados |
| Block pruning | Evitar blocos irrelevantes |
| Ordem das colunas | Priorizar filtro mais frequente |

Python 3.10+ e custo local zero; BigQuery cobra armazenamento e consultas. Validado `customer_id` e `product_id` nas primeiras posições.

## Tecnologias relacionadas ainda não utilizadas
Sem dataset, INFORMATION_SCHEMA, bytes reais, particionamento, benchmark ou deploy.

## Referências oficiais
- [Consultar tabelas clusterizadas](https://cloud.google.com/bigquery/docs/querying-clustered-tables)
- [Criar tabelas clusterizadas](https://cloud.google.com/bigquery/docs/creating-clustered-tables)
...
cd "009-bigquery-clustering-advisor"
```

Confirme que `pwd` termina em `009-bigquery-clustering-advisor`. Os caminhos relativos usados pelo código dependem disso.

# Detector de hotspots em row keys do Bigtable

> GCP 007 · intermediário · NoSQL de baixa latência

Detecta row keys iniciadas pela mesma data, padrão que concentra escritas sequenciais, e sugere um bucket determinístico antes do sensor e do tempo.

![Arquitetura](docs/arquitetura.svg)

## Executar e testar
```bash
python3 -m src.analyzer
...
| Item | Função |
|---|---|
| Python 3, CSV e SHA-256 | Análise e bucket determinístico |
| Cloud Bigtable | Banco wide-column futuro |
| Row key | Índice primário e ordenação lexicográfica |
| Prefix scan | Padrão eficiente de leitura por intervalo |
| Hash bucket | Distribuir escritas no espaço de chaves |
| Hotspot share | Medir concentração do prefixo líder |
| `unittest` e SVG | Teste e documentação visual |

## Conceitos aplicados
- modelagem orientada às consultas;
- ordenação lexicográfica e range reads;
- hotspot causado por timestamp no início;
- salting/bucketing e seu custo para consultas.

## Pré-requisitos, custos e validação
Python 3.10+, custo local zero. Foi detectado prefixo temporal em 100% das escritas e geradas alternativas com oito buckets. Bigtable gera custos de nós/compute e armazenamento quando criado.

## Tecnologias relacionadas ainda não utilizadas
Não há instância, SDK, tablets, Key Visualizer, column families, garbage collection, autoscaling ou benchmark real.

## Referências oficiais
- [Boas práticas de schema no Bigtable](https://cloud.google.com/bigtable/docs/schema-design)

## Publicação
Rascunho local; nenhuma tabela ou instância GCP foi criada.
...
cd "007-bigtable-rowkey-hotspot"
```

Confirme que `pwd` termina em `007-bigtable-rowkey-hotspot`. Os caminhos relativos usados pelo código dependem disso.

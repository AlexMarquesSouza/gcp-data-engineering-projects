# Dataform incremental com qualidade de dados

> GCP 003 · intermediário · Dataform e BigQuery

Inclui uma definição SQLX de tabela incremental particionada e assertions de unicidade, preenchimento e valor positivo. Um validador Python demonstra as falhas localmente.

![Arquitetura](docs/arquitetura.svg)

## Executar e testar
```bash
python3 -m src.assertions
...
| Item | Função |
|---|---|
| Dataform SQLX | Definir transformação incremental |
| BigQuery | Executar SQL e armazenar tabela particionada numa evolução cloud |
| `uniqueKey` | Permitir merge e testar duplicidades |
| Assertions | Testar unicidade, nulos e regras de negócio |
| Python 3 | Prévia local das regras |
| `unittest` e SVG | Teste e documentação visual |

## Conceitos aplicados
- ELT e transformação SQL versionada;
- tabela incremental e filtro pelo maior dia carregado;
- chave única, partição e qualidade como código;
- assertion falha quando retorna registros inválidos.

## Pré-requisitos, custos e validação
Python 3.10+, custo local zero. Foram detectados `o-2` duplicado e valor negativo na linha 5. Dataform e BigQuery podem gerar custos somente quando executados no GCP.

## Tecnologias relacionadas ainda não utilizadas
Não há workspace Dataform, dataset, IAM, compilação, execução, CI/CD, full refresh ou custo de consulta. O SQLX é somente um artefato para revisão.

## Referências oficiais
- [Criar tabelas incrementais no Dataform](https://cloud.google.com/dataform/docs/create-tables)
- [Testar qualidade com assertions](https://cloud.google.com/dataform/docs/assertions)

## Publicação
Rascunho local; nenhum workflow ou repositório remoto foi publicado.
...
cd "003-dataform-incremental-qualidade"
```

Confirme que `pwd` termina em `003-dataform-incremental-qualidade`. Os caminhos relativos usados pelo código dependem disso.

# Consumidor idempotente com dead-letter

> GCP 002 · iniciante/intermediário · Pub/Sub e processamento de eventos

Simula reentregas de mensagens, impede que o mesmo `message_id` produza efeito duas vezes e envia eventos inválidos para uma dead-letter local.

![Arquitetura Pub/Sub](docs/arquitetura.svg)
...
| Item | Função |
|---|---|
| Python 3 e JSON Lines | Consumidor e lote de mensagens local |
| Estado JSON | Registro de IDs processados |
| Google Cloud Pub/Sub | Serviço de mensageria da evolução cloud |
| Pull subscription | Modelo compatível com exactly-once do Pub/Sub |
| Dead-letter topic | Destino de mensagens que não podem ser processadas |
| `unittest` e SVG | Testes e explicação visual |

## Conceitos aplicados
- entrega pelo menos uma vez, reentrega e deduplicação;
- consumidor idempotente e persistência de progresso;
- validação, acknowledgment conceitual e dead-letter;
- separação entre erro de negócio e duplicidade.

## Pré-requisitos, custos e validação
Python 3.10+, custo local zero. Foram validados dois sucessos, uma duplicidade, uma mensagem inválida e reexecução sem reaplicar sucessos. Pub/Sub gera custos somente numa implantação futura.

## Tecnologias relacionadas ainda não utilizadas
Não há projeto GCP, tópico, assinatura, IAM, cliente Pub/Sub, ack real, tentativas configuráveis, concorrência ou Cloud Monitoring.

## Referências oficiais
- [Entrega exactly-once no Pub/Sub](https://cloud.google.com/pubsub/docs/exactly-once-delivery)
- [Dead-letter topics](https://cloud.google.com/pubsub/docs/dead-letter-topics)

## Publicação
Rascunho local; nenhum tópico, assinatura, recurso pago ou repositório remoto foi criado.
...
cd "002-consumidor-idempotente-pubsub"
```

Confirme que `pwd` termina em `002-consumidor-idempotente-pubsub`. Os caminhos relativos usados pelo código dependem disso.
